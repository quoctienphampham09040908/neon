		...
		<div >
			<form  action="<?= str_replace('act=edit','act=save',$_SERVER['REQUEST_URI']) ?>" method="post"  >
				...
				<div class="col-md-6 form-box">
					...
					<img class="img-thumbnail" id="thumbnail-preview" src="../<?= $category['thumbnail'] ?>" alt="">
					<input type="hidden" value="<?= $category['thumbnail'] ?>" id="thumbnail" name="thumbnail">
					<a class="btn btn-dark" href="javascript:void(0)" onclick="set_Image('#thumbnail-preview','#thumbnail');" >Chọn hình</a>
				</div>
				<? foreach ($attr['textarea'] as $key => $value) { ?>
				<div class="col-md-6 form-box">
					<label for="<?= $value?>"><?= $key ?></label>
					<textarea id="<? $value?>" name="<?= $value ?>" class="form-control form-control-custom-admin" ><?= $category[$value] ?></textarea>
				</div>
				<? 	} ?>
