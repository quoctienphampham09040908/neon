...
$attr = array();
$db->query("select * from table_category where type like 'product'");
$attr = array();
...
$attr['default']['category'] = array(
    "category_id_default" => array("title" => "Danh mục sản phẩm", "list" => $db->result_array() )
);
...
switch ($act) {
  case 'list':
    $limit = 10;
    $db->query("select p.*,(select title from table_category c where c.id = p.category_id ) as `title_category` from table_product p ");
  ...
  case 'edit':
    if (!empty($id)) {
      $db->query("select * from table_product where id like '{$id}' ");
      $item = $db->fetch_array(); // get table default
      $db->query("select * from table_product_translate  where product_id = '{$id}'");
