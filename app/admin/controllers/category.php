...
switch ($act) {
  case 'list':
    ...
    $db->query("select * from table_category where {$where} ");
    $category_list = $db->result_array();
    $paging = new Paging(array("items" => $category_list, "limit" => $limit, "p" => isset($_REQUEST['p']) ? $_REQUEST['p'] : "", "url" => getCurrentUrl()));
    $category_list = $paging->paging(true);
  ...
  case 'edit':
    if (!empty($id)) {
      ...
      $db->query("select * from table_category_translate  where category_id = '{$id}'");
